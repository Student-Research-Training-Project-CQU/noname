...
extern uint16_t g_safe_distance_max ;
extern uint16_t g_gradient_start ;
extern uint16_t g_yellow_point ;
extern uint16_t g_orange_point ;
extern uint16_t g_red_point ;
extern uint16_t g_critical_threshold ;
...
{
    if (distance == 0 || distance >= g_safe_distance_max)
    {
        // 非常安全 → 暗绿
        *r = 0;
        *g = 40;
    ...
    float danger = 0.0f;
    if (distance <= g_red_point)
    {
        danger = 1.0f;
    }
    else if (distance < g_gradient_start)
    {
        uint16_t danger_range = 1;
        if (g_gradient_start > g_red_point) {
            danger_range = (uint16_t)(g_gradient_start - g_red_point);
        }
        danger = (float)(g_gradient_start - distance) / (float)danger_range;
    }
    // danger 现在是 0.0 ~ 1.0
...
bool LED_Thresholds_Update(uint16_t safe, uint16_t caution, uint16_t warning)
{
    ...
    g_safe_distance_max = safe;
    g_gradient_start = safe;
    g_yellow_point = caution;
    g_orange_point = warning;
    g_red_point = (uint16_t)(warning / 2U);
    if (g_red_point == 0) {
        g_red_point = 1;
    }
    g_critical_threshold = (uint16_t)(g_red_point / 2U);
    if (g_critical_threshold == 0) {
        g_critical_threshold = 1;
    }
    return true;
}
